...
# ℙ[M | τ]
struct AlignmentDistribution <: DiscreteMultivariateDistribution
...
function _rand!(rng::AbstractRNG, d::AlignmentDistribution,
    ...
    len = max_length + 1
    seq_lengths = fill(0, num_descendants(d.τ)+1)

    # reject samples which exceed match length or ones which have null sequences
    while len > max_length || any(seq_lengths .< 3)
        i = 1
        s = START_INDEX
        while s != END_INDEX
        ...
        len = i-1
        seq_lengths = [count(M[j, 1:len] .== 1) for j ∈ eachindex(seq_lengths)]
    end
    M[:, (len+1):end] .= 0
    ids[(len+1):end] .= 0
